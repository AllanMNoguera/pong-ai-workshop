...
# We create a PongEnv class that will handle the game logic and rendering
# The class will have methods to reset the game, get the current state, take a step, and render the game
# Each step will return the observation, reward, and done flag
class PongEnv:
    def __init__(self):
        pygame.init()
        ...
        # We can adjust the speed of the opponent paddle by changing the speed multiplier
    ...
    def get_observation(self):
        ...
        return np.array(
            [
                ...
                self.ball.x,
                self.ball.y,  # Ball position
                self.ball_speed_x,
                self.ball_speed_y,  # Ball velocity
            ],
            dtype=np.float32,
        )
    ...
    def step(self, action):
        ...
        if self.ball.colliderect(self.paddle_left):
            self.ball_speed_x = (
                self.check_paddle_collision(self.paddle_left, self.ball_speed_x) * -1
            )

        if self.ball.colliderect(self.paddle_right):
            self.ball_speed_x = (
                self.check_paddle_collision(self.paddle_right, self.ball_speed_x) * -1
            )
    ...
    def check_paddle_collision(self, paddle, ball_speed_x):
        ...
        ball_y = (
            self.ball.centery
        )  # The vertical position of the ball at the time of collision
        ...
        edge_threshold = (
            paddle.height * 0.2
        )  # 20% of the paddle height considered as edge
        ...
        if distance_from_top < edge_threshold or distance_from_bottom < edge_threshold:
            ...
            self.ball_speed_y += (
                distance_from_center / paddle.height
            ) * 10  # Scale vertical speed
    ...
    def render(self):
        ...
        pygame.display.flip()
        self.clock.tick(60)
        # We call pygame.event.pump() so we can detect long press of the keys
        pygame.event.pump()

    def close(self):
        pygame.quit()

# We can test the PongEnv class by running it in a main function
# This will create a Pong environment and play the game for 1000 frames
if __name__ == "__main__":
    env = PongEnv()
    state = env.reset()

    for _ in range(1000):  # Play for 1000 frames
        # Here the action is not controlled by an agent, but by random selection
        # Exercice: Implement a random selection of actions (0, 1, 2) and observe the game
        state, reward, done = env.step(TODO)
        env.render()
